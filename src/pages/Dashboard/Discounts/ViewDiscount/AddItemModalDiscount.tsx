            ...
            {product.variations.map((item: any) => {
              return (
                <div className=" related_product_flex" key={item.id}>
                  <div className="left_container">
                    <Checkbox
                      checked={variationIdList.includes(item.id)}
                      onChange={(e) => {
                    ...
                    <img
                      src={
                        item.image ? `${IMAGEURL}${item.image}` : alt_image_url
                      }
                      alt="product"
                    />
                  </div>
...
export const AddItemModalDiscount = ({
  ...
  const [addItem, { isLoading: loadAdd }] = useAddItemDiscountMutation();
  const resetFnc = () => {
    setProductList([]);
    setProductIdList([]);
    setVariationIdList([]);
  ...
  const handleSubmit = async ({ exclude = false }: { exclude: boolean }) => {
    const discountProductActiveIds = productList
      .filter((item: any) => item.has_discount)
      .map((item: any) => item.id);
    const discountVariationActiveIds = variationList
      .filter((item: any) => item.has_discount)
      .map((item: any) => item.id);
    ...
    if (exclude) {
      payload = {
        products: productIdList.filter(
          (item: any) => !discountProductActiveIds.includes(item)
        ),
        product_variations: variationIdList.filter(
          (item: any) => !discountVariationActiveIds.includes(item)
        ),
