...
const ExtraLocationModal = ({ openModal, closeModal }: Props) => {
  ...
  const makePayment = async () => {
    const payload = {
      ...
      plan_id: 14,
      plan_count: count,
      redirect_url: `${REDIRECT_URL}dashboard/store/location/create?slot=${count}&success=true`,
    };
    try {
  ...
  return (
    ...
    >
      <div className={`extra_wrapper staff ${step === 2 ? "step2_staff" : ""}`}>
        ...
        <img src="/xlocation.svg" alt="" />
        {step == 1 && (
          <div className="text_content">
            <h3>Add more store location</h3>
            <p>
              Each additional location costs N100,000 and comes with 3 extra
              staff slots & other Pro features.
            </p>
            <p className="label">How many locations do you want to add?</p>
            <InputField
              type="number"
              name="locationCount"
              value={count}
        ...
        {step == 2 && amount && (
          <div className="text_content">
            <h3>
              Amount to pay for <span>{count}</span> extra location
              {count && count > 1 && "s"}
            </h3>
            <p>
              Location costs and expiration are calculated based on your current
              subscription cycle.
            </p>
...
export default ExtraLocationModal;
